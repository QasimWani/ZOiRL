...
class SequentialDomainReductionTransformer(DomainTransformer):
    ...
    def __init__(
        self, gamma_osc: float = 0.7, gamma_pan: float = 1.0, eta: float = 0.9
    ) -> None:
...
class BayesianOptimization(Observable):
    def __init__(
        self, f, pbounds, random_state=None, verbose=2, bounds_transformer=None
    ):
...
class Agent(TD3):
    ...
    def __init__(self, **kwargs):
        ...
        self.all_costs = [
            [[] for _ in range(self.candidate_num)] for bid in range(self.buildings)
        ]
        self.per_period = 7
        # self.DT_costs = []
        ...
        self.zeta_k_list = np.ones(
            ((self.candidate_num, 24, self.buildings))
        )  # 4 different Zetas.
        ...
        self.bounds_transformer = [
            SequentialDomainReductionTransformer() for _ in range(self.buildings)
        ]
        self.optimizer = [
            BayesianOptimization(
                f=None,
                pbounds={
                    f"p_ele{i}": (self.range_p_ele[0], self.range_p_ele[1])
                    for i in range(24)
                },
                verbose=2,
                random_state=0,
                bounds_transformer=self.bounds_transformer[bid],
            )
            for bid in range(self.buildings)
        ]
        self.utility = [
            UtilityFunction(kind="ei", kappa=0.1, xi=1e-4)
            for _ in range(self.buildings)
        ]
        self.upbounds = [self.range_p_ele[1] * np.ones((24, self.buildings))]
        self.lobounds = [self.range_p_ele[0] * np.ones((24, self.buildings))]
        self.best_ind_cum = [
            np.zeros(self.candidate_num) for _ in range(self.buildings)
        ]
    ...
    def evaluate_and_update(self, state):
        ...
        if self.total_it % 24 == 0:  # Calculate cost at the end of the day
            ...
            if len(self.all_costs[0][self.candidate_ind]) >= self.per_period:
                self.candidate_ind = (self.candidate_ind + 1) % self.candidate_num
                ...
                if (
                    ...
                    for bid in range(self.buildings):
                        ...
                        for can_ind in range(self.candidate_num):
                            self.optimizer[bid].register(
                                params={
                                    f"p_ele{i}": self.zeta_k_list[can_ind, i, bid]
                                    + (np.random.rand(1)[0] - 0.5) * 1e-5
                                    for i in range(24)
                                },
                                target=all_cost_t[can_ind],
                            )
                    ...
                    upbounds_t = np.zeros((24, self.buildings))
                    lobounds_t = np.zeros((24, self.buildings))
                    # for bid in range(self.buildings):
                    #     x_min_bound = [b[0] for b in self.bounds_transformer[bid].bounds[-1]]
                    #     x_max_bound = [b[1] for b in self.bounds_transformer[bid].bounds[-1]]
                    #     upbounds_t[:, bid] = np.array(x_max_bound)
                    #     lobounds_t[:, bid] = np.array(x_min_bound)
                    # x = self.optimizer[bid].space.params[-1]
                    # print(f'B{bid} upbounds:')
                    # print(np.array(x_max_bound))
                    # print(f'B{bid} lobounds:')
                    # print(np.array(x_min_bound))
                    # print(f'B{bid} last values:')
                    # print(np.array(x))

                    self.upbounds.append(upbounds_t)
                    self.lobounds.append(lobounds_t)
                    ...
                    for bid in range(self.buildings):
                        ...
                        if (
                            self.total_it >= 24 * 60
                        ):  # only start doing this when we see enough samples

                            next_point = self.optimizer[bid].suggest(self.utility[bid])
                            p_ele_suggest = np.array(
                                [next_point[f"p_ele{i}"] for i in range(24)]
                            )
                            self.zeta_k_list[0, :, bid] = p_ele_suggest
                            print(f"B{bid} starts evaluation with BayesOpt")
                            print(p_ele_suggest)
                        ...
                        best_candidate.append(best_ind[0])
                        self.best_ind_cum[bid][best_ind[0]] = (
                            self.best_ind_cum[bid][best_ind[0]] + 1
                        )
                        ...
                        best_zeta_avg = self.zeta_k_list[best_ind[0], :, bid]
                        ...
                        # ] = self.p_ele_step
                        # adj_zeta_avg = adj_zeta_avg - np.mean(adj_zeta_avg)
                        # self.zeta_k_list[3, :, bid] = best_zeta_avg + adj_zeta_avg*factor
                        print(f"B{bid} Best candidate at iteration {self.total_it}")
                        print(self.best_ind_cum[bid])
                    ...
                    print(f"Best candidate at iteration {self.total_it}")
