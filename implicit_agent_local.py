...
params = {
    "data_path": Path("data/Climate_Zone_5_Optim"),
...
def estimate_data(
    surrogate_env: CityLearn, data: dict, t_start: int, init_updates: dict
):
...
while not done and t_idx < end_time:
    ...
    if t_idx % 1460 < rbc_threshold:
        ...
        if t_idx % 24 == 0:  # first hour
            ...
            optim_results = get_actions(
                data_est, t_idx % 24, lookahead=True
            )  # day-ahead plan
            action_planned_day, cost_dispatch, action_planned = zip(*optim_results)

            look_ahead_cost.append(
                cost_dispatch
            )  # per day estimation cost after solving for hour 1 for hours 1-24
        # action = get_actions(data_est, t_idx % 24) #runs optimization per hour.
        assert (
            len(action_planned_day[0][0]) == 24
        ), "Invalid number of observations for Optimization actions"

        actions = [
            np.array(action_planned_day[idx])[:, t_idx % 24]
            for idx in range(len(actions_spaces))
        ]
...
for i in range(3):
    for j in range(3):
        ...
        axs[i, j].plot(
            E_grid[bid][week:], label="True E grid: Optim"
        )  # plot true E grid
        axs[i, j].plot(
            E_grid_pred[bid][week:] + E_grid_sell_pred[bid][week:],
            "gx",
...
for key_i in range(len(env_comp_item)):
    ...
    for i in range(3):
        for j in range(3):
            ...
            axs[i, j].plot(
                data_np[bid][week:], label="optimization"
            )  # plot true E grid
...
for key_i in range(len(env_comp_item)):
    ...
    for i in range(3):
        for j in range(3):
            ...
            axs[i, j].plot(data_env[week:], label="environment")  # plot true E grid
            axs[i, j].plot(
                data_env2[week:], label="environment SOC"
            )  # plot true E grid
