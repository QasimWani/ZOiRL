...
for i in range(time_sim):
    CEM_E_grid_t =np.sum(E_grid_true[(RBC_THRESHOLD+i*24):(RBC_THRESHOLD+(i+1)*24),:],axis = 1)
    RBC_Egrid_t = np.sum(E_grid_RBC[(RBC_THRESHOLD+i*24):(RBC_THRESHOLD+(i+1)*24),:],axis = 1)
    ...
    tot_cost_ratio_agg.append(0.5*(np.max(CEM_E_grid_t)/np.max(RBC_Egrid_t)+np.sum(np.abs(CEM_E_grid_t[1:] - CEM_E_grid_t[:-1]))/np.sum(np.abs(RBC_Egrid_t[1:] - RBC_Egrid_t[:-1]))))
