...
class Critic:
	def __init__(self, t:int, parameters:dict, building_id:int, num_actions:int):
        ...
        # Alphas TODO
        # alpha_peak1 = cp.Parameter(name='E_ns', shape=(window), value=parameters['E_ns'][t:, building_id])
        # alpha_peak2 = cp.Parameter(name='H_bd', shape=(window), value=parameters['H_bd'][t:, building_id])
        # alpha_ramp = cp.Parameter(name='C_bd', shape=(window), value=parameters['C_bd'][t:, building_id])
    ...
    def forward(self, debug=False, dispatch=False):
        ...
        if float('-inf') < Q_reward_warping_output < float('inf'):
            G_t_tPlusn = self.get_past_rewards(t) + [Q_reward_warping_output] ## takes past rewards nd.array of shape T - t - 1 {! need to define this !}
            G_t_lambda = (1 - self.lambda_) * np.sum([self.lambda_**(i - 1) * G_t_tPlusn[t : t + i] for i in range(len(G_t_tPlusn)) ]) + self.lambda_**(24 - t - 1) * G_t_tPlusn[t]
    ...
    def target_update(self, data_target, data_local):
        _alpha_ramp, _alpha_peak1, _alpha_peak2 = zip(*self.backward(data_local, t, building_id).values()) ## local update after Least square optim.
        ### main target update
