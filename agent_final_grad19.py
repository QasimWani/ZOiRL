...
class Agent(TD3):
    ...
    def __init__(self, **kwargs):
        ...
        self.all_costs = [[[] for _ in range(self.num_candidate)] for bid in range(self.buildings)]
        self.per_period = 7
        # self.DT_costs = []
        ...
        self.zeta_k_list = np.ones(((self.num_candidate , 24, self.buildings)))  # 4 different Zetas.
            ...
            if len(self.all_costs[0][self.candidate_ind]) >= self.per_period:
                ...
                if (
                    ...
                    for bid in range(9):
                        ...
                        self.zeta_k_list[1, :, bid] = best_zeta_avg + adj_zeta_avg*factor
                        ...
                        adj_zeta_avg = adj_zeta_avg - np.mean(adj_zeta_avg)
                        self.zeta_k_list[2, :, bid] = best_zeta_avg + adj_zeta_avg*factor
...
class Predictor(DataLoader):
        ...
        for day in range(range_index, 0):
            for uid in self.building_ids:
                ...
                if self.carbon_avg[uid].all() != 0:
                    self.carbon_avg[uid] = self.carbon_avg[uid] * 0.7 + carbon_inten[uid] * 0.3
                else:
    ...
    def infer_solar_electricity_load(self, timestep: int):
        ...
        day_pred_carbon = {uid: np.zeros([window], dtype=np.float32) for uid in self.building_ids}
