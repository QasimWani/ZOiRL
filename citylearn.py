...
def auto_size(buildings):
    for building in buildings.values():
        ...
        if building.dhw_heating_device.nominal_power == "autosize":
            ...
            if isinstance(building.dhw_heating_device, HeatPump):
                ...
                building.dhw_heating_device.nominal_power = np.array(
                    building.sim_results["dhw_demand"]
                    / building.dhw_heating_device.cop_heating
                ).max()
            ...
            # If the device is an electric heater
                ...
                building.dhw_heating_device.nominal_power = (
                    ...
                    / building.dhw_heating_device.efficiency
                ).max()
        ...
        building.dhw_storage.capacity = (
            max(building.sim_results["dhw_demand"]) * building.dhw_storage.capacity
        )
        building.cooling_storage.capacity = (
            max(building.sim_results["cooling_demand"])
            * building.cooling_storage.capacity
        )
...
def building_loader(
    ...
    for uid, attributes in zip(data, data.values()):
        if uid in building_ids:
            ...
            building.sim_results["non_shiftable_load"] = list(
                data["Equipment Electric Power [kWh]"]
            )
            ...
            building.sim_results["rh_out"] = list(
                weather_data["Outdoor Relative Humidity [%]"]
            )
            building.sim_results["diffuse_solar_rad"] = list(
                weather_data["Diffuse Solar Radiation [W/m2]"]
            )
            building.sim_results["direct_solar_rad"] = list(
                weather_data["Direct Solar Radiation [W/m2]"]
            )

            # Reading weather forecasts
            building.sim_results["t_out_pred_6h"] = list(
                weather_data["6h Prediction Outdoor Drybulb Temperature [C]"]
            )
            ...
            building.sim_results["rh_out_pred_6h"] = list(
                weather_data["6h Prediction Outdoor Relative Humidity [%]"]
            )
            building.sim_results["rh_out_pred_12h"] = list(
                weather_data["12h Prediction Outdoor Relative Humidity [%]"]
            )
            building.sim_results["rh_out_pred_24h"] = list(
                weather_data["24h Prediction Outdoor Relative Humidity [%]"]
            )
            ...
            building.sim_results["solar_gen"] = list(
                attributes["Solar_Power_Installed(kW)"]
                * data["Hourly Data: AC inverter power (W)"]
                / 1000
            )
                ...
                if value == True:
                    if state_name == "net_electricity_consumption":
                        ...
                        _net_elec_cons_upper_bound = max(
                            ...
                            + np.array(building.sim_results["dhw_demand"]) / 0.8
                            + np.array(building.sim_results["cooling_demand"])
                            + building.dhw_storage.capacity / 0.8
                        ...
                        if state_name in [
                            ...
                            s_low_central_agent.append(
                                min(building.sim_results[state_name])
                            )
                            s_high_central_agent.append(
                                max(building.sim_results[state_name])
                            )
                ...
                if value == True:
                    if action_name == "cooling_storage":
                        ...
                        if attributes["Chilled_Water_Tank"]["capacity"] > 0.000001:
                            a_low.append(
                                max(
                                    -1.0 / attributes["Chilled_Water_Tank"]["capacity"],
                                    -1.0,
                                )
                            )
                            a_high.append(
                            ...
                            a_low_central_agent.append(
                                max(
                                    -1.0 / attributes["Chilled_Water_Tank"]["capacity"],
                                    -1.0,
                                )
                            )
                            a_high_central_agent.append(
                                min(
                                    1.0 / attributes["Chilled_Water_Tank"]["capacity"],
                                    1.0,
                                )
                        ...
                        if attributes["DHW_Tank"]["capacity"] > 0.000001:
                            a_low.append(
                                max(-1.0 / attributes["DHW_Tank"]["capacity"], -1.0)
                            )
                            a_high.append(
                                min(1.0 / attributes["DHW_Tank"]["capacity"], 1.0)
                            )
                            a_low_central_agent.append(
                                max(-1.0 / attributes["DHW_Tank"]["capacity"], -1.0)
                            )
    ...
    observation_space_central_agent = spaces.Box(
        low=np.float32(np.array(s_low_central_agent)),
        high=np.float32(np.array(s_high_central_agent)),
        dtype=np.float32,
    )
    action_space_central_agent = spaces.Box(
        low=np.float32(np.array(a_low_central_agent)),
    ...
    for building in buildings.values():
        ...
        if isinstance(building.dhw_heating_device, HeatPump):
            ...
            building.dhw_heating_device.cop_heating = (
                building.dhw_heating_device.eta_tech
            ...
            building.dhw_heating_device.cop_heating[
                building.dhw_heating_device.cop_heating < 0
            ] = 20.0
            building.dhw_heating_device.cop_heating[
                building.dhw_heating_device.cop_heating > 20
            ] = 20.0
            building.dhw_heating_device.cop_heating = (
                building.dhw_heating_device.cop_heating.to_numpy()
            )

        building.cooling_device.cop_cooling = (
            building.cooling_device.eta_tech
        ...
        building.cooling_device.cop_cooling[
            building.cooling_device.cop_cooling < 0
        ] = 20.0
        building.cooling_device.cop_cooling[
            building.cooling_device.cop_cooling > 20
        ] = 20.0
        building.cooling_device.cop_cooling = (
            building.cooling_device.cop_cooling.to_numpy()
        )
    ...
    return (
        ...
        observation_spaces,
        action_spaces,
        observation_space_central_agent,
        action_space_central_agent,
    )
...
class CityLearn(gym.Env):
    def __init__(
        ...
        self.costs_periodic = []  # measures cost overtime
    ...
    def get_building_information(self):
        ...
        for uid, building in self.buildings.items():
            ...
            building_info[uid]["solar_power_capacity (kW)"] = round(
                building.solar_power_capacity, 3
            )
            building_info[uid]["Annual_DHW_demand (kWh)"] = round(
                sum(building.sim_results["dhw_demand"]) / n_years, 3
            )
            building_info[uid]["Annual_cooling_demand (kWh)"] = round(
                sum(building.sim_results["cooling_demand"]) / n_years, 3
            )
            building_info[uid]["Annual_nonshiftable_electrical_demand (kWh)"] = round(
                sum(building.sim_results["non_shiftable_load"]) / n_years, 3
            )
            ...
            for uid_corr, building_corr in self.buildings.items():
                if uid_corr != uid:
                    building_info[uid]["Correlations_DHW"][uid_corr] = round(
                        (
                            np.corrcoef(
                                np.array(building.sim_results["dhw_demand"]),
                                np.array(building_corr.sim_results["dhw_demand"]),
                            )
                        )[0][1],
                        3,
                    )
                    building_info[uid]["Correlations_cooling_demand"][uid_corr] = round(
                        (
                            np.corrcoef(
                                np.array(building.sim_results["cooling_demand"]),
                                np.array(building_corr.sim_results["cooling_demand"]),
                            )
                        )[0][1],
                        3,
                    )
                    building_info[uid]["Correlations_non_shiftable_load"][
                        ...
                        (
                            np.corrcoef(
                                np.array(building.sim_results["non_shiftable_load"]),
                                np.array(
                                    building_corr.sim_results["non_shiftable_load"]
                                ),
                            )
                        )[0][1],
                        3,
    ...
    def step(self, actions):
        ...
        self.buildings_net_electricity_demand = []
        self.current_carbon_intensity = list(self.buildings.values())[0].sim_results[
            "carbon_intensity"
        ][self.time_step]
        electric_demand = 0
        ...
        if self.central_agent:
            ...
            for uid, building in self.buildings.items():
                ...
                self.buildings_net_electricity_demand.append(
                    -building_electric_demand
                )  # >0 if solar generation > electricity consumption
            ...
            for a, (uid, building) in zip(actions, self.buildings.items()):
                ...
                assert sum(
                    self.buildings_states_actions[uid]["actions"].values()
                ) == len(a), (
                    "The number of input actions for building "
                    + str(uid)
                    + " must match the number of actions defined in the list of building attributes."
                )
                    ...
                    if self.buildings_states_actions[uid]["actions"]["cooling_storage"]:
                        ...
                        if self.buildings_states_actions[uid]["actions"]["dhw_storage"]:
                            ...
                            _electric_demand_electrical_storage = (
                                building.set_storage_electrical(a[2])
                            )
                        ...
                        # 'Electrical Storage' & 'Cooling Storage'
                            ...
                            # Electrical
                            _electric_demand_electrical_storage = (
                                building.set_storage_electrical(a[1])
                            )
                            elec_consumption_electrical_storage += (
                                _electric_demand_electrical_storage
                            )
                        ...
                        # 'Electrical Storage'
                            ...
                            # Electrical
                            _electric_demand_electrical_storage = (
                                building.set_storage_electrical(a[0])
                            )
                            elec_consumption_electrical_storage += (
                                _electric_demand_electrical_storage
                            )
        ...
        if self.central_agent:
                    ...
                    if value == True:
                        if state_name not in s_appended:
                            if state_name in [
                                ...
                                s.append(
                                    building.cooling_storage._soc
                                    / building.cooling_storage.capacity
                                )
                            elif state_name == "dhw_storage_soc":
                                s.append(
                                    building.dhw_storage._soc
                                    / building.dhw_storage.capacity
                                )
                ...
                for state_name, value in self.buildings_states_actions[uid][
                    ...
                    if value == True:
                        if state_name == "net_electricity_consumption":
                            ...
                            s.append(
                                building.electrical_storage._soc
            ...
            rewards = self.reward_function.get_rewards(
                self.buildings_net_electricity_demand, self.current_carbon_intensity
            )
        ...
        self.carbon_emissions.append(
            np.float32(max(0, electric_demand) * self.current_carbon_intensity)
        )
        ...
        if (
            self.time_step % self.cost_analysis == 0
            and self.time_step > 0
            and not terminal
        ):  # run cost analysis
            cost = self.force_cost_analysis()
            self.costs_periodic.append(cost)
    ...
    def reset(self):
        ...
        if self.central_agent:
            ...
            self.reward_function = reward_function_ma(
                len(self.building_ids), self.get_building_information()
            )
    ...
    def _terminal(self):
        ...
        if is_terminal:
            ...
            self.electric_consumption_electric_storage = np.array(
                self.electric_consumption_electric_storage
            )
    ...
    def convert_to_list(self):
        self.carbon_emissions = list(self.carbon_emissions)
        self.net_electric_consumption = list(self.net_electric_consumption)
        self.net_electric_consumption_no_storage = list(
            self.net_electric_consumption_no_storage
        )
        self.net_electric_consumption_no_pv_no_storage = list(
            self.net_electric_consumption_no_pv_no_storage
        )
        self.electric_consumption_electric_storage = list(
            self.electric_consumption_electric_storage
        )
        self.electric_consumption_dhw_storage = list(
            self.electric_consumption_dhw_storage
        )
        self.electric_consumption_cooling_storage = list(
            self.electric_consumption_cooling_storage
        )
        self.electric_consumption_dhw = list(self.electric_consumption_dhw)
        self.electric_consumption_cooling = list(self.electric_consumption_cooling)
        self.electric_consumption_appliances = list(
            self.electric_consumption_appliances
        )
        self.electric_generation = list(self.electric_generation)
    ...
    def get_baseline_cost_buildings(self):
        """Returns cost for all building using RBC agent"""
        building_costs = {i: defaultdict(dict) for i in self.building_ids}

        for building_id, building in self.buildings.items():
            if isinstance(building.net_electric_consumption, list):
                building.net_electric_consumption = np.array(
                    building.net_electric_consumption
                )

            if "ramping" in self.cost_function:
                building_costs[building_id]["cost"]["ramping"] = np.abs(
                    (
                        building.net_electric_consumption
                        - np.roll(building.net_electric_consumption, 1)
                    )[1:]
                ).sum()

            if self.simulation_period[1] - self.simulation_period[0] > 8760:
                building_costs[building_id]["cost_last_year"][
                    "ramping_last_yr"
                ] = np.abs(
                    (
                        building.net_electric_consumption[-8760:]
                        - np.roll(building.net_electric_consumption[-8760:], 1)
                    )[1:]
                ).sum()

            if "1-load_factor" in self.cost_function:
                building_costs[building_id]["cost"]["1-load_factor"] = np.mean(
                    [
                        1
                        - np.mean(
                            building.net_electric_consumption[i : i + int(8760 / 12)]
                        )
                        / np.max(
                            building.net_electric_consumption[i : i + int(8760 / 12)]
                        )
                        for i in range(
                            0, len(building.net_electric_consumption), int(8760 / 12)
                        )
                    ]
                )
            ...
            if "average_daily_peak" in self.cost_function:
                building_costs[building_id]["cost"]["average_daily_peak"] = np.mean(
                    [
                        building.net_electric_consumption[i : i + 24].max()
                        for i in range(0, len(building.net_electric_consumption), 24)
                    ]
                )

                if self.simulation_period[1] - self.simulation_period[0] > 8760:
                    building_costs[building_id]["cost_last_year"][
                        "average_daily_peak_last_yr"
                    ] = np.mean(
                        [
                            building.net_electric_consumption[-8760:][i : i + 24].max()
                            for i in range(
                                0, len(building.net_electric_consumption[-8760:]), 24
                            )
                        ]
                    )

            if "peak_demand" in self.cost_function:
                building_costs[building_id]["cost"][
                    "peak_demand"
                ] = building.net_electric_consumption.max()

                if self.simulation_period[1] - self.simulation_period[0] > 8760:
                    building_costs[building_id]["cost_last_year"][
                        "peak_demand_last_yr"
                    ] = self.net_electric_consumption[-8760:].max()

            if "net_electricity_consumption" in self.cost_function:
                building_costs[building_id]["cost"][
                    "net_electricity_consumption"
                ] = building.net_electric_consumption.clip(min=0).sum()

                if self.simulation_period[1] - self.simulation_period[0] > 8760:
                    building_costs[building_id]["cost_last_year"][
                        "net_electricity_consumption_last_yr"
                    ] = (building.net_electric_consumption[-8760:].clip(min=0).sum())

            if "carbon_emissions" in self.cost_function:
                building_costs[building_id]["cost"][
                    "carbon_emissions"
                ] = self.carbon_emissions.sum()
            ...
            if "quadratic" in self.cost_function:
                building_costs[building_id]["cost"]["quadratic"] = (
                    building.net_electric_consumption.clip(min=0) ** 2
                ).sum()
                ...
                if self.simulation_period[1] - self.simulation_period[0] > 8760:
                    building_costs[building_id]["cost_last_year"][
                        ...
                        building.net_electric_consumption[-8760:].clip(min=0) ** 2
                    ).sum()

        return building_costs

    def cost_detailed_view(self):
        """Computes costs for individual buildings"""
        # Running the reference rule-based controller to find the baseline cost
        if self.cost_rbc_detailed is None:
            env_rbc = CityLearn(
                self.data_path,
            ...
            while not done:
                action = agent_rbc.select_action(
                    [
                        list(env_rbc.buildings.values())[0].sim_results["hour"][
                            env_rbc.time_step
                        ]
                    ]
                )
        ...
        for id, building in self.buildings.items():
            if "ramping" in self.cost_function:
                building_costs[id]["cost"]["ramping"] = (
                    ...
                    / self.cost_rbc_detailed[id]["cost"]["ramping"]
                )
                building_costs[id]["cost"]["c_score"] = [
                    building_costs[id]["cost"]["ramping"]
                ]
            ...
            if "1-load_factor" in self.cost_function:
                building_costs[id]["cost"]["1-load_factor"] = (
                    np.mean(
                        [
                            1
                            - np.mean(
                            ...
                            for i in range(
                                0,
                                len(building.net_electric_consumption),
                                int(8760 / 12),
                            )
                        ]
                    )
                    / self.cost_rbc_detailed[id]["cost"]["1-load_factor"]
                )
                building_costs[id]["cost"]["c_score"].append(
                    building_costs[id]["cost"]["1-load_factor"]
                )
                ...
                if self.simulation_period[1] - self.simulation_period[0] > 8760:
                    building_costs[id]["cost_last_year"]["1-load_factor_last_yr"] = (
                        np.mean(
                            [
                                ...
                                for i in range(
                                    0,
                                    len(building.net_electric_consumption[-8760:]),
                                    int(8760 / 12),
                                )
                            ]
                        )
                    ...
                    building_costs[id]["cost_last_year"]["c_score_last_year"].append(
                        building_costs[id]["cost_last_year"]["1-load_factor_last_yr"]
                    )
            ...
            if "average_daily_peak" in self.cost_function:
                building_costs[id]["cost"]["average_daily_peak"] = (
                    np.mean(
                        [
                            building.net_electric_consumption[i : i + 24].max()
                            for i in range(
                                0, len(building.net_electric_consumption), 24
                            )
                        ]
                    )
                    / self.cost_rbc_detailed[id]["cost"]["average_daily_peak"]
                )
                building_costs[id]["cost"]["c_score"].append(
                    building_costs[id]["cost"]["average_daily_peak"]
                )
                ...
                if self.simulation_period[1] - self.simulation_period[0] > 8760:
                    building_costs[id]["cost_last_year"][
                        "average_daily_peak_last_yr"
                    ] = (
                        np.mean(
                            [
            ...
            if "peak_demand" in self.cost_function:
                building_costs[id]["cost"]["peak_demand"] = (
                    building.net_electric_consumption.max()
                    / self.cost_rbc_detailed[id]["cost"]["peak_demand"]
                )
                building_costs[id]["cost"]["c_score"].append(
                    building_costs[id]["cost"]["peak_demand"]
                )
                ...
                if self.simulation_period[1] - self.simulation_period[0] > 8760:
                    building_costs[id]["cost_last_year"]["peak_demand_last_yr"] = (
                        building.net_electric_consumption[-8760:].max()
                        / self.cost_rbc_detailed[id]["cost_last_year"][
                            "peak_demand_last_yr"
                        ]
                    )
            ...
            if "net_electricity_consumption" in self.cost_function:
                building_costs[id]["cost"]["net_electricity_consumption"] = (
                    building.net_electric_consumption.clip(min=0).sum()
                    / self.cost_rbc_detailed[id]["cost"]["net_electricity_consumption"]
                )
            ...
            if "carbon_emissions" in self.cost_function:
                building_costs[id]["cost"]["carbon_emissions"] = (
                    self.carbon_emissions.sum()
                    / self.cost_rbc_detailed[id]["cost"]["carbon_emissions"]
                )
            ...
            # compute total cost
            arr = []
            for k, c in building_costs[id]["cost"].items():
                if k != "c_score":
                    arr.append(c)
            building_costs[id]["cost"]["total"] = np.mean(arr)

            if building_costs[id]["cost"]["c_score"] != []:
                building_costs[id]["cost"]["coordination_score"] = np.mean(
                    building_costs[id]["cost"]["c_score"]
                )

            if (
                self.simulation_period[1] - self.simulation_period[0] > 8760
                and building_costs[id]["cost_last_year"]["c_score_last_year"] != []
            ):
                building_costs[id]["cost_last_year"][
                    "coordination_score_last_yr"
                ] = np.mean(building_costs[id]["cost_last_year"]["c_score_last_year"])

                # compute total cost
                arr = []
                for k, c in building_costs[id]["cost_last_year"].items():
                    if k != "c_score_last_year":
                        arr.append(c)
                building_costs[id]["cost_last_year"]["total_last_yr"] = np.mean(arr)
    ...
    def cost(self, simulation_period):
        ...
        if simulation_period[1] - simulation_period[0] > 8760:
            ...
            self.cost_rbc = env_rbc.get_baseline_cost(simulation_period)
        ...
        if "ramping" in self.cost_function:
            cost["ramping"] = (
                ...
                / self.cost_rbc["ramping"]
            )
            c_score.append(cost["ramping"])

            if simulation_period[1] - simulation_period[0] > 8760:
                cost_last_yr["ramping_last_yr"] = (
                    np.abs(
                        (
                            self.net_electric_consumption[-8760:]
                    ...
                    / self.cost_rbc_last_yr["ramping_last_yr"]
                )
        ...
        if "1-load_factor" in self.cost_function:
            ...
            if simulation_period[1] - simulation_period[0] > 8760:
                cost_last_yr["1-load_factor_last_yr"] = (
                    np.mean(
                        [
                            1
                            - np.mean(
                    ...
                    / self.cost_rbc_last_yr["1-load_factor_last_yr"]
                )
                c_score_last_yr.append(cost_last_yr["1-load_factor_last_yr"])

        # Average of all the daily peaks of the 365 day of the year. The peaks are calculated using the net energy demand of the whole district of buildings.
        if "average_daily_peak" in self.cost_function:
            cost["average_daily_peak"] = (
                np.mean(
                    [
                ...
                / self.cost_rbc["average_daily_peak"]
            )
            c_score.append(cost["average_daily_peak"])

            if simulation_period[1] - simulation_period[0] > 8760:
                cost_last_yr["average_daily_peak_last_yr"] = (
                    np.mean(
                        [
                    ...
                    / self.cost_rbc_last_yr["average_daily_peak_last_yr"]
                )
        ...
        if "peak_demand" in self.cost_function:
            cost["peak_demand"] = (
                self.net_electric_consumption.max() / self.cost_rbc["peak_demand"]
            )
            c_score.append(cost["peak_demand"])

            if simulation_period[1] - simulation_period[0] > 8760:
                cost_last_yr["peak_demand_last_yr"] = (
                    self.net_electric_consumption[-8760:].max()
        ...
        # Positive net electricity consumption for the whole district. It is clipped at a min. value of 0 because the objective is to minimize the energy consumed in the district, not to profit from the excess generation. (Island operation is therefore incentivized)
        if "net_electricity_consumption" in self.cost_function:
            cost["net_electricity_consumption"] = (
                self.net_electric_consumption.clip(min=0).sum()
            ...
            if simulation_period[1] - simulation_period[0] > 8760:
                cost_last_yr["net_electricity_consumption_last_yr"] = (
                    ...
                    / self.cost_rbc_last_yr["net_electricity_consumption_last_yr"]
                )

        if "carbon_emissions" in self.cost_function:
            cost["carbon_emissions"] = (
                self.carbon_emissions.sum() / self.cost_rbc["carbon_emissions"]
            )

            if simulation_period[1] - simulation_period[0] > 8760:
                cost_last_yr["carbon_emissions_last_yr"] = (
                    self.carbon_emissions[-8760:].sum()
        ...
        if "quadratic" in self.cost_function:
            cost["quadratic"] = (
                self.net_electric_consumption.clip(min=0) ** 2
            ).sum() / self.cost_rbc["quadratic"]
            c_score.append(cost["quadratic"])

            if simulation_period[1] - simulation_period[0] > 8760:
                cost_last_yr["quadratic_last_yr"] = (
                    self.net_electric_consumption[-8760:].clip(min=0) ** 2
                ).sum() / self.cost_rbc_last_yr["quadratic_last_yr"]
    ...
    def get_baseline_cost(self, simulation_period):
        ...
        if "1-load_factor" in self.cost_function:
            ...
            if simulation_period[1] - simulation_period[0] > 8760:
                cost_last_yr["1-load_factor_last_yr"] = np.mean(
                    [
                        1
                        - np.mean(
                            self.net_electric_consumption[-8760:][
