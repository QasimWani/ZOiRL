...
class Actor:
    ...
    def create_problem(self, t: int, parameters: dict, building_id: int):
        ...
        p_ele = cp.Parameter(
            name="p_ele",
            shape=(window),
            value=self.zeta["p_ele"][
                :, building_id
            ],  # we need to pass it with the rolled value
        )
        ...
        E_grid_pkhist = cp.Parameter(
            name="E_grid_pkhist",
            value=np.max(
                [0, *parameters["E_grid"][:, building_id]]
        ...
        E_ns = cp.Parameter(
            name="E_ns",
            shape=window,
            value=parameters["E_ns"][:, building_id],  # [t:, building_id]
        )
        H_bd = cp.Parameter(
            name="H_bd",
            shape=window,
            value=parameters["H_bd"][:, building_id],  # [t:, building_id]
        )
        C_bd = cp.Parameter(
            name="C_bd",
            shape=window,
            value=parameters["C_bd"][:, building_id],  # [t:, building_id]
        )

        # PV generations
        E_pv = cp.Parameter(
            name="E_pv", shape=window, value=parameters["E_pv"][:, building_id]
        )

        # Heat Pump
        COP_C = cp.Parameter(
            name="COP_C", shape=window, value=parameters["COP_C"][:, building_id]
        )
        ...
        eta_bat = cp.Parameter(
            name="eta_bat", shape=window, value=self.zeta["eta_bat"][:, building_id]
        )
        ...
        eta_Hsto = cp.Parameter(
            name="eta_Hsto",
            shape=window,
            value=self.zeta["eta_Hsto"][:, building_id],
        )
        ...
        eta_Csto = cp.Parameter(
            name="eta_Csto",
            shape=window,
            value=self.zeta["eta_Csto"][:, building_id],
        )
        ...
        self.constraints.append(
            ...
            + E_hpC
            + E_ehH
            + (action_bat + np.roll(self.rbc_actions["action_bat"][building_id, :], -t))
            * C_p_bat
        )  # electricity balance
        self.constraints.append(
            E_ehH * eta_ehH + H_bal_relax
            == (action_H + np.roll(self.rbc_actions["action_H"][building_id, :], -t))
            * C_p_Hsto
            + H_bd
        )  # heat balance
        ...
        self.constraints.append(
            ...
            == (action_C + np.roll(self.rbc_actions["action_C"][building_id, :], -t))
            * C_p_Csto
            + C_bd
        )  # cooling balance
        ...
        self.constraints.append(
            ...
            + (action_bat[0] + self.rbc_actions["action_bat"][building_id, t])
            * eta_bat[0]
            + SOC_Brelax[0]
        )  # initial SOC
        # soc updates
        for i in range(1, window):
            self.constraints.append(
                ...
                + (
                    action_bat[i]
                    + self.rbc_actions["action_bat"][building_id, (t + i) % 24]
                )
                * eta_bat[i]
                + SOC_Brelax[i]
            )
        ...
        self.constraints.append(
            ...
            + (action_H[0] + self.rbc_actions["action_H"][building_id, t]) * eta_Hsto[0]
            + SOC_Hrelax[0]
        )  # initial SOC
        # soc updates
        for i in range(1, window):
            self.constraints.append(
                ...
                + (
                    action_H[i]
                    + self.rbc_actions["action_H"][building_id, (t + i) % 24]
                )
                * eta_Hsto[i]
                + SOC_Hrelax[i]
            )
        ...
        self.constraints.append(
            ...
            + (action_C[0] + self.rbc_actions["action_C"][building_id, t]) * eta_Csto[0]
            + SOC_Crelax[0]
        )  # initial SOC
        # soc updates
        for i in range(1, window):
            self.constraints.append(
                ...
                + (
                    action_C[i]
                    + self.rbc_actions["action_C"][building_id, (t + i) % 24]
                )
                * eta_Csto[i]
                + SOC_Crelax[i]
            )
        ...
        for high, low in zip(bounds_high.items(), bounds_low.items()):
            ...
            # Battery action
                ...
                self.constraints.append(
                    action_bat
                    + np.roll(self.rbc_actions["action_bat"][building_id, :], -t)
                    <= h
                )
                self.constraints.append(
                    action_bat
    ...
    def inject_params(self, t: int, parameters: dict, building_id: int):
        ...
        ### --- Parameters ---
        problem_parameters["p_ele"].value = np.roll(
            self.zeta["p_ele"][:, building_id], -t
        )
        ...
        problem_parameters["E_grid_pkhist"].value = np.max(
            [0, *parameters["E_grid"][:, building_id]]
        )

        # Loads
        problem_parameters["E_ns"].value = parameters["E_ns"][:, building_id]
        problem_parameters["H_bd"].value = parameters["H_bd"][:, building_id]
        problem_parameters["C_bd"].value = parameters["C_bd"][:, building_id]

        # PV generations
        problem_parameters["E_pv"].value = parameters["E_pv"][:, building_id]

        # Heat Pump
        problem_parameters["COP_C"].value = parameters["COP_C"][:, building_id]
        ...
        problem_parameters["eta_bat"].value = np.roll(
            self.zeta["eta_bat"][:, building_id], -t
        )
        ...
        problem_parameters["eta_Hsto"].value = np.roll(
            self.zeta["eta_Hsto"][:, building_id], -t
        )
        ...
        problem_parameters["eta_Csto"].value = np.roll(
            self.zeta["eta_Csto"][:, building_id], -t
        )
    ...
    def forward(
        ...
        return (
            [
                0.95 * actions["action_C"]
                + 0.05 * self.rbc_actions["action_C"][building_id, t],
                0.95 * actions["action_H"]
                + 0.05 * self.rbc_actions["action_H"][building_id, t],
                0.95 * actions["action_bat"]
                + 0.05 * self.rbc_actions["action_bat"][building_id, t],
            ],
            actions,  # debug
            actions["E_grid"] + actions["E_grid_sell"],
        )
...
class Predictor(DataLoader):
    ...
    def __init__(self, building_info: dict, action_space: list) -> None:
        ...
        self.daytype_record = 2 * np.ones(24, dtype=int)
    ...
    def get_day_data(self, replay_buffer: ReplayBuffer, timestep: int):
        ...
        observation_data = {}
        if timestep % 24 == 0:
            self.daytype_record[0] = self.state_buffer.get(-1)["daytype"][0][0]
            self.daytype_record = np.roll(self.daytype_record, -1)
        ...
        len_egc = egc.shape[0]
        egc_prev = np.array(self.state_buffer.get(-2)["elec_cons"])
        len_egc_prev = 24 - len_egc
        E_grid = np.append(egc_prev[-len_egc_prev:, :], egc[:, :], axis=0)
        observation_data["E_grid"] = E_grid
        #
        ...
        for hour in range(1, timestep % 24 + 1):
            observation_data["E_grid_prevhour"][hour] = egc[hour]
    ...
    def gather_input(self, timestep):
        ...
        buffer_2 = self.state_buffer.get(-1)
        for i in range(len(buffer_2["diffuse_24h"])):
            buffer["diffuse_24h"].append(buffer_2["diffuse_24h"][i])
            buffer["direct_24h"].append(buffer_2["direct_24h"][i])
            buffer["t_out_24h"].append(buffer_2["t_out_24h"][i])
        ...
        for uid in self.building_ids:
            ...
            x_diffuse_24h = np.array(buffer["diffuse_24h"])[-24:, uid]
            # x_direct_6h = np.array(buffer["direct_6h"])[-6:, uid]
            # x_direct_12h = np.array(buffer["direct_12h"])[-6:, uid]
            x_direct_24h = np.array(buffer["direct_24h"])[-24:, uid]
            ...
            input_solar_full[uid][:, 0] = x_diffuse_24h
            input_solar_full[uid][:, 1] = x_direct_24h

            # x_elec_6h = np.array(buffer["t_out_6h"])[-6:, uid]
            # x_elec_12h = np.array(buffer["t_out_12h"])[-6:, uid]
            x_elec_24h = np.array(buffer["t_out_24h"])[-24:, uid]

            # input_elec_full[uid][0:6, 0] = x_elec_6h
            # input_elec_full[uid][6:12, 0] = x_elec_12h
            input_elec_full[uid][:, 0] = x_elec_24h

            input_solar[uid][:, :] = input_solar_full[uid]
            input_elec[uid][:, :] = input_elec_full[uid]
    ...
    def infer_solar_electricity_load(self, timestep: int):
        ...
        elec_dem = np.zeros([T])
        current_elec_dem = np.zeros([T])

        daytype = pred_buffer.get(-1)["daytype"][0][0]
        ...
        z = self.daytype_record[range(-7, -len(self.daytype_record), -7)]
        next_daytype = np.bincount(z).argmax()
        current_day_left = 24 - timestep % 24
        next_day_left = 24 - current_day_left

        for uid in self.building_ids:

            if daytype == 7:
                elec_dem[:current_day_left] = np.roll(
                    self.elec_weekend1_avg[uid], -(timestep % 24)
                )[:current_day_left]
                current_elec_dem = self.elec_weekend1_avg[uid]
            elif daytype in [1, 8]:
                elec_dem[:current_day_left] = np.roll(
                    self.elec_weekend2_avg[uid], -(timestep % 24)
                )[:current_day_left]
                current_elec_dem = self.elec_weekend2_avg[uid]
            else:
                elec_dem[:current_day_left] = np.roll(
                    self.elec_weekday_avg[uid], -(timestep % 24)
                )[:current_day_left]
                current_elec_dem = self.elec_weekday_avg[uid]
            ...
            if timestep % 24 in [1, 2, 3, 4, 5]:
                """say it is hour 5, we calculate avg for (0, 5) which correspond to the current"""
                offset_e1 = np.sum(today_e_load[-day_hour - 1 :, uid]) / (day_hour + 1)
                offset_e2 = np.sum(current_elec_dem[: day_hour + 1]) / (day_hour + 1)
                offset_e = offset_e1 - offset_e2
                day_pred_elec[uid] = elec_dem + offset_e
                ...
                offset_e2 = np.sum(current_elec_dem[day_hour - 4 : day_hour + 1]) / 5
                offset_e = offset_e1 - offset_e2
                day_pred_elec[uid] = elec_dem + offset_e
    ...
    def infer_heating_cooling_estimate(self, timestep):
        ...
        **so that when we obtain from ReplayBuffer.get_recent(), we get day-long data.
        ...
        current_day_left = 24 - timestep % 24
        next_day_left = 24 - current_day_left
        # print(f"now daytype: {daytype}, next daytype: {next_daytype}")
        ...
        for uid in self.building_ids:
            ...
            # ------------use average-------------
            if daytype == 7:
                est_h_load[uid][:current_day_left] = np.roll(
                    self.h_weekend1_avg[uid], -(timestep % 24)
                )[:current_day_left]
                est_c_load[uid][:current_day_left] = np.roll(
                    self.c_weekend1_avg[uid], -(timestep % 24)
                )[:current_day_left]
            elif daytype in [1, 8]:
                est_h_load[uid][:current_day_left] = np.roll(
                    self.h_weekend2_avg[uid], -(timestep % 24)
                )[:current_day_left]
                est_c_load[uid][:current_day_left] = np.roll(
                    self.c_weekend2_avg[uid], -(timestep % 24)
                )[:current_day_left]
            else:
                est_h_load[uid][:current_day_left] = np.roll(
                    self.h_weekday_avg[uid], -(timestep % 24)
                )[:current_day_left]
                est_c_load[uid][:current_day_left] = np.roll(
                    self.c_weekday_avg[uid], -(timestep % 24)
                )[:current_day_left]
