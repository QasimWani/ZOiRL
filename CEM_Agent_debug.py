...
class CEM_Agent_debug(object):
    def __init__(
        ...
        self.actor = Actor(num_actions, num_buildings, rbc_threshold)
        self.actor_norl = deepcopy(
            self.actor
        )  # NORL actor, i.e. actor whose parameters stay constant.

        # Instantiate the Digital Twin Class
        ## self.DigitalTwin = DigitalTwin(eta_hp_tech: float = 0.22,   # Technical Efficiency
    ...
    def get_zeta(self):  # Getting zeta for the 9 buildings for 24 hours
        ...
        if len(self.elite_set_prev) and self.k <= self.K_keep:
            ...
            self.zeta = self.elite_set_prev[
                self.k
            ]  # k-th best from elite_set_prev - zeta for 9 buildings
    ...
    # TODO: UPDATE ACCORDINGLY -- see comments (highlighted as NOTE)
    ...
    def select_action(self, state, env: CityLearn = None, day_ahead: bool = True):
        ...
        if self.total_it >= self.rbc_threshold:
            ...
            if (
                self.total_it % 24 == 0
            ):  # Get zeta at the starting of the day to be used for the rest of the day

                # Setting 9 zetas for 9 different buildings
                zeta_k = self.get_zeta()  ### NOTE: get zeta -- put into actor
                ...
                #                 self.p_ele_logger.append(zeta_k)
                self.elite_set.append(
                    zeta_k
                )  ### NOTE: store zeta's - called in `set_EliteSet_EliteSetPrev`

                #                 print(np.shape(self.elite_set))

                # Setting zeta for each of the 9 buildings
                ### NOTE: Setting zeta for each of the 9 buildings - calls `actor.set_zeta`
                for i in range(9):
                    zeta_tuple = (
                        zeta_k[0, :, i],
                        self.zeta_eta_bat[:, :, i],
            ...
            ### NOTE: -- Abstracted --- NO NEEED FOR THIS. End of function
        ...
        ### NOTE: COST EVALUATION
        if (
            self.total_it >= self.rbc_threshold and self.total_it % 24 >= 0
        ):  # Storing the value of variables to calculate end of the day cost

            # Getting current states for all the 9 buildings

            E_observed = state[:, 28]  # For 9 buildings

            E_NS_t = state[:, 23]  # For 9 buildings

            data_output = self.memory.get(-1)  ### NOTE: MAKE SURE THIS WORKS
            ...
            self.E_netelectric_hist.append(
                E_observed
            )  # List of 24 lists each list size 9
        ...
        if (
            self.total_it >= self.rbc_threshold and self.total_it % 24 == 23
        ):  # Calculate cost at the end of the day
        ...
        ### NOTE: END of cost evaluaiton --- abstract into another function
    ...
    def get_cost_day_end(self):
        ...
        print(np.shape(self.outputs["C_bd_hist"]))
        print(np.shape(self.outputs["H_bd_hist"]))
        print(np.shape(self.outputs["COP_C_hist"]))
    ...
    def set_EliteSet_EliteSetPrev(self):
        ...
        if self.k > self.N_samples:  # Enough samples of zeta collected
            ...
            #             print('Means are getting changed')
