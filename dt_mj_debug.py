...
while not done and env.time_step < end_time:
    ...
    action_next = agents.select_action_debug(
        next_state, False
...
week = end_time - 24 * 5  # plots last week of the month data
...
for i in range(3):
    for j in range(3):
        ...
        axs[i, j].plot(
            E_grid_true[bid][week:], label="True E grid"
        )  # plot true E grid
        axs[i, j].plot(
            E_grid_dt[bid][week:], label="Digital twin Egrid"
        )  # plot true E grid
        axs[i, j].grid()
        if j == 0:
