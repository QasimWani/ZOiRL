...
for key_i in range(len(env_comp_item)):
    ...
    for i in range(3):
        for j in range(3):
            ...
            axs[i, j].plot(
                data_np[bid][-sthour:]
                * check_data_param[env_comp_item_check3[key_i]][bid][0],
                label="optimization",
            )  # plot true E grid
            axs[i, j].plot(
                data_np2[bid][-sthour:]
            ...
            axs[i, j].scatter(
                np.arange(-1, plot_days * 24 - 1, 24),
                np.array(
                    check_data_param[env_comp_item_check4[key_i]][bid][-plot_days:]
                )
                * float(check_data_param[env_comp_item_check3[key_i]][bid][0]),
                s=50,
                marker="x",
                label="optimization init SOC",
            )
            axs[i, j].scatter(
                np.arange(0, plot_days * 24, 24),
                (
                    np.array(
                        check_data_param[env_comp_item_check4[key_i]][bid][-plot_days:]
                    )
                    + data_np[bid][
                        np.array(
                            range(week - RBC_THRESHOLD, end_time - RBC_THRESHOLD, 24)
                        )
                    ]
                )
                * float(check_data_param[env_comp_item_check3[key_i]][bid][0]),
...
for key_i in range(len(env_comp_item)):
    ...
    plt.legend()
    fig.savefig(
        f"images/{env_comp_item_check[key_i]}_optim-diff-env_plot.pdf",
        bbox_inches="tight",
    )
...
ax1.plot(
    np.sum(Optim_cost["total_cost"], axis=0) / np.sum(RBC_cost["total_cost"], axis=0),
    label=f"Optim/RBC",
)  # plot true E grid
ax1.grid()
