...
class Agent(TD3):
    ...
    def __init__(self, **kwargs):
        ...
        self.a_bat_adj = np.ones(((self.num_candidate, 24, self.buildings)))
        self.a_csto_adj = np.ones(((self.num_candidate, 24, self.buildings)))
        self.a_bat_step = 0.015  # temp
        self.a_csto_step = 0.015  # temp
            ...
            if len(self.all_costs[0][self.candidate_ind]) >= self.per_period:
                ...
                if (
                    ...
                    for bid in range(9):
                        ...
                        best_bat_avg = 0.5 * np.squeeze(
                            self.a_bat_adj[best_ind[0], :, bid]
                            + self.a_bat_adj[best_ind[1], :, bid]
                        )
                        best_csto_avg = 0.5 * np.squeeze(
                            self.a_csto_adj[best_ind[0], :, bid]
                            + self.a_csto_adj[best_ind[1], :, bid]
                        )

                        self.best_p_ele[bid].append(best_zeta_avg)
                        self.zeta_k_list[0, :, bid] = best_zeta_avg
                        self.a_bat_adj[0, :, bid] = best_bat_avg
                        self.a_csto_adj[0, :, bid] = best_csto_avg
                        ...
                        adj_bat_avg = np.zeros(24)
                        adj_bat_avg[
                            np.intersect1d(high_ind[:6], high_ind_total[:6])
                        ] = -self.a_bat_step
                        adj_bat_avg = adj_bat_avg - np.mean(adj_bat_avg)
                        self.a_bat_adj[1, :, bid] = best_bat_avg + adj_bat_avg * factor

                        adj_csto_avg = np.zeros(24)
                        adj_csto_avg[
                            np.intersect1d(high_ind[:6], high_ind_total[:6])
                        ] = -self.a_csto_step
                        adj_csto_avg = adj_csto_avg - np.mean(adj_csto_avg)
                        self.a_csto_adj[1, :, bid] = (
                            best_csto_avg + adj_csto_avg * factor
                        )
                        ...
                        adj_bat_avg = adj_bat_avg - np.mean(adj_bat_avg)
                        self.a_bat_adj[2, :, bid] = best_bat_avg + adj_bat_avg * factor
                ...
                self.set_zeta(
                    self.zeta_k_list[self.candidate_ind, :, :],
                    self.a_bat_adj[self.candidate_ind, :, :],
                    self.a_csto_adj[self.candidate_ind, :, :],
                )

        # self.mean_elite_set.append(self.mean_p_ele)

    def set_zeta(self, zeta=None, a_bat_adj=None, a_csto_adj=None):
...
class Actor:
    def __init__(
        ...
        self.prob = [
            [None] * 24 for i in range(self.num_buildings)
        ]  # template for each hour, per building (9x24)
        ...
        self.rbc_actions_adj = {
            "action_C": np.zeros((9, 24)),
            "action_H": np.zeros((9, 24)),
            "action_bat": np.zeros((9, 24)),
        }
    ...
    def initialize_zeta(
        ...
        zeta["a_bat_adj"] = np.full((24, self.num_buildings), a_bat_adj)
        zeta["a_csto_adj"] = np.full((24, self.num_buildings), a_csto_adj)
    ...
    def create_problem(self, t: int, parameters: dict, building_id: int):
        ...
        a_bat_adj = cp.Parameter(
            name="a_bat_adj",
            shape=(window),
            value=self.zeta["a_bat_adj"][t:, building_id],
        )

        a_csto_adj = cp.Parameter(
            name="a_csto_adj",
            shape=(window),
            value=self.zeta["a_csto_adj"][t:, building_id],
        )
        ...
        eta_bat = self.zeta["eta_bat"][t:, building_id]
        ...
        C_p_Hsto = parameters["C_p_Hsto"][t, building_id]

        eta_Hsto = self.zeta["eta_Hsto"][t:, building_id]
        #     name="eta_Hsto",
        #     shape=window,
        ...
        C_p_Csto = parameters["C_p_Csto"][t, building_id]

        eta_Csto = self.zeta["eta_Csto"][t:, building_id]
        ...
        self.constraints.append(
            ...
            + (
                ...
                + self.rbc_actions_adj["action_bat"][building_id, T - window :]
            )
            * C_p_bat
        )  # electricity balance
        self.constraints.append(
            ...
            == (
                action_H
                + self.rbc_actions["action_H"][building_id, T - window :]
                + self.rbc_actions_adj["action_H"][building_id, T - window :]
            )
            * C_p_Hsto
            + H_bd
        )  # heat balance
        ...
        self.constraints.append(
            ...
            == (
                action_C
                + self.rbc_actions["action_C"][building_id, T - window :]
                + self.rbc_actions_adj["action_C"][building_id, T - window :]
            )
            * C_p_Csto
            + C_bd
        )  # cooling balance
    ...
    def get_problem(self, t: int, parameters: dict, building_id: int):
        ...
        # Form objective.
        if self.prob[building_id][t] is None:
            self.create_problem(
                t, parameters, building_id
            )  # problem formulation for Actor optimizaiton
            obj = cp.Minimize(self.cost)
            # Form problem.
            self.prob[building_id][t] = cp.Problem(obj, self.constraints)
            assert self.prob[building_id][t].is_dpp()
        else:  # DPP
    ...
    def inject_params(self, t: int, parameters: dict, building_id: int):
        ...
        assert (
            self.prob[building_id][t] is not None
        ), "Problem must be defined to be able to use DPP."
        problem_parameters = self.prob[building_id][t].param_dict

        ### --- Parameters ---
        problem_parameters["p_ele"].value = self.zeta["p_ele"][t:, building_id]
        problem_parameters["a_bat_adj"].value = self.zeta["a_bat_adj"][t:, building_id]
        problem_parameters["a_csto_adj"].value = self.zeta["a_csto_adj"][
            t:, building_id
        ]
        ...
        for key, prob_val in problem_parameters.items():
            self.prob[building_id][t].param_dict[key].value = prob_val.value
    ...
    def forward(
        ...
        actions = {}
        try:
            status = self.prob[building_id][t].solve(
        ...
        if float("-inf") < status < float("inf"):
            for var in self.prob[building_id][t].variables():
                if dispatch:
                    offset = np.zeros(len(var.value))
                    if "action" in str(var.name()):
                        offset = (
                            self.rbc_actions[var.name()][building_id, 24 - t % 24 :]
                            + self.rbc_actions_adj[var.name()][
                                building_id, 24 - t % 24 :
                            ]
                        )
                    ...
                    if "action" in str(var.name()):
                        offset = (
                            self.rbc_actions[var.name()][building_id, t % 24]
                            + self.rbc_actions_adj[var.name()][building_id, t % 24]
                        )
    ...
    def set_zeta(
        ...
        self.zeta["a_bat_adj"][:, building_id] = a_bat_adj
        self.zeta["a_csto_adj"][:, building_id] = a_csto_adj
        # dimensions: 1
